...
public class TaskTableUI extends UI {
	...
	protected void init(VaadinRequest request) {
        ...
        final ComboBox stateFiled = new ComboBox("Status");
        stateFiled.addItem(State.TODO);
        stateFiled.addItem(State.IN_PROGRESS);
        stateFiled.addItem(State.DONE);
	...
	private PagedFilterTable<IndexedContainer> buildPagedFilterTable() {
        ...
        /*filterTable.setVisibleColumns(new String[] { "name", "id", "status",
    ...
    private Container buildContainer() {
        ...
        cont.addContainerProperty("status", State.class, null);
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            cont.getContainerProperty(i, "name").setValue("Task " + i);
            cont.getContainerProperty(i, "description").setValue("Description " + i);
            
            Random r = new Random();
            cont.getContainerProperty(i, "priority").setValue(r.nextInt(4) + 1);
            /* Set state property */
            int rndInt = random.nextInt(3);
            State stateToSet = State.TODO;
            if (rndInt == 0) {
                stateToSet = State.IN_PROGRESS;
            } else if (rndInt == 1) {
                stateToSet = State.DONE;
            }
            cont.getContainerProperty(i, "status").setValue(stateToSet);
            /* Set date property */
	...
	private Component buildButtons(final FilterTable relatedFilterTable) {
        ...
        Button setVal = new Button("Set the Status filter to 'IN PROGRESS'");
        setVal.addClickListener(new Button.ClickListener() {

            public void buttonClick(ClickEvent event) {
                relatedFilterTable
                        .setFilterFieldValue("status", State.IN_PROGRESS);
            }
        });
